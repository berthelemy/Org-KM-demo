---
title: ClearWave X1 Handset (Budget)
aliases:
  - X1 Budget
  - X1 Standard
tags:
  - product
  - telecoms
  - handset
  - smartphone
  - budget
product-type: telecoms
status: active
launched: 2025-09-01
discontinued: ""
provider: ClearWave Mobile
related:
  - "[[PROD-ClearWaveX1ProHandset]]"
  - "[[PROD-SIMOnly10GB5G]]"
  - "[[PROD-SIMOnlyUnlimited5G]]"
---

## Overview

The ClearWave X1 is ClearWave Mobile's entry-level Android smartphone, designed for customers who want a capable 5G device at an accessible price. It is well-suited to first-time smartphone buyers, older customers, and those who use their phone primarily for calls, messaging, social media, and light browsing.

## Key Features

- 6.5" IPS LCD display, 90 Hz
- Dual-camera system: 50 MP main / 5 MP ultrawide
- 8 MP front camera
- 4 nm quad-core processor
- 4 GB RAM / 128 GB storage (expandable via microSD up to 512 GB)
- 4,500 mAh battery; 25W fast charge
- 5G, Wi-Fi 5, Bluetooth 5.2, NFC
- IP52 splash resistant
- Android 15; 3 years of OS updates guaranteed
- Colour options: Slate Grey, Pearl White

## Eligibility

- Available to new and upgrading ClearWave Mobile customers
- Full credit check required for contract purchases
- Minimum age: 18
- Handset purchase only (no SIM): available without credit check at full price

## Pricing & Terms

**With 24-month SIM plan:**

| Bundle | Monthly SIM | Monthly Handset | Total Monthly | Upfront |
|--------|-------------|-----------------|---------------|---------|
| 10 GB 5G + X1 | £12.00 | £10.00 | £22.00 | £0.00 |
| Unlimited 5G + X1 | £22.00 | £10.00 | £32.00 | £0.00 |

**Handset standalone:**

| Option | Price |
|--------|-------|
| Full outright price | £249.00 |
| Finance (0% APR, 24 months) | £10.38/month |

- 24-month manufacturer warranty included
- Accidental damage cover available as add-on (£4.99/month)

## Regulatory Considerations

- Consumer credit: handset financing is a regulated credit agreement under the Consumer Credit Act 1974 — agents must present the full credit agreement cost before the customer commits
- Cooling-off period: **14 calendar days** for remotely sold contracts
- Do not describe IP52 as "waterproof" — it is splash resistant only; full submersion will likely cause damage

## Frequently Asked Questions

**Q: Is this phone good for older or less tech-confident customers?**
A: Yes. Android 15 includes simplified accessibility features, large text options, and an easy-mode launcher. The 6.5" screen is also well-suited to customers with visual difficulties.

**Q: How does it compare to the X1 Pro?**
A: The X1 is ideal for everyday tasks — calls, texts, social media, photos, and browsing. The [[PROD-ClearWaveX1ProHandset]] offers a significantly better camera, more RAM, wireless charging, and a higher-refresh display. If the customer doesn't need those features, the X1 is excellent value.

**Q: Can I expand the storage?**
A: Yes — a microSD card up to 512 GB can be added. This is useful for customers who store lots of photos or offline music.

**Q: Is NFC available for contactless payments?**
A: Yes. The X1 supports Google Pay and other NFC payment services.

## Common Objections

| Objection | Suggested Response |
|-----------|-------------------|
| "It seems too basic" | For everyday use — calls, messaging, social media, photos — the X1 does everything the vast majority of customers need. The 5G connection means it's future-proof too. |
| "Will it get slow after a year?" | 4 GB of RAM handles everyday apps well. With 128 GB of storage expandable via microSD, it won't run out of space either. |
| "I'd rather pay a bit more for something better" | The [[PROD-ClearWaveX1ProHandset]] is a significant step up in camera and performance — the monthly difference on a 24-month plan is £20/month. |

---

## Related

- [[PROD-ClearWaveX1ProHandset]]
- [[PROD-SIMOnly10GB5G]]
- [[PROD-SIMOnlyUnlimited5G]]
