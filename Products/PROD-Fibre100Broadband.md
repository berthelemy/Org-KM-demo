---
title: Fibre 100 Broadband
aliases:
  - Fibre 100
  - Entry Fibre
tags:
  - product
  - telecoms
  - broadband
  - fibre
product-type: telecoms
status: active
launched: 2024-01-15
discontinued: ""
provider: ClearWave
related:
  - "[PROD-Fibre500Broadband](PROD-Fibre500Broadband.md)"
  - "[PROD-Fibre1000Broadband](PROD-Fibre1000Broadband.md)"
  - "[PROD-HomePhone](PROD-HomePhone.md)"
---

## Overview

Fibre 100 is ClearWave's entry-level full-fibre broadband product, designed for households with light-to-moderate internet usage — typically 1–3 users streaming, browsing, and working from home. It delivers download speeds of up to 100 Mbps over a dedicated fibre-to-the-premises (FTTP) connection.

## Key Features

- Average download speed: **100 Mbps**
- Average upload speed: **15 Mbps**
- Full-fibre (FTTP) — no copper in the connection
- Unlimited data usage
- Includes ClearWave Smart Hub 3 router
- 24-month contract (18-month also available at higher monthly price)
- No mid-contract price rises for the length of the initial term

## Eligibility

- Available to residential customers in ClearWave FTTP coverage areas
- Credit check required; subject to address availability check
- Not available on existing copper (ADSL/VDSL) infrastructure — FTTP must be present at the premises
- Minimum age: 18

## Pricing & Terms

| Item | Amount |
|------|--------|
| Monthly charge (24-month) | £28.00 |
| Monthly charge (18-month) | £32.00 |
| Upfront activation fee | £0.00 |
| Early termination charge | Remaining months × monthly charge |
| Router replacement | £49.99 |
| Engineer visit (avoidable fault) | £65.00 |

- Price correct as of May 2026; subject to change after initial contract term
- Direct Debit payment required; surcharge of £2.50/month for other payment methods

## Regulatory Considerations

- Regulated by Ofcom under the Communications Act 2003
- Agents must provide the **Key Facts Indicator (KFI)** document before sale completes — do not skip this step
- Cooling-off period: **14 calendar days** from contract start; customer may cancel without penalty
- Speed claims must reference **"average"** speeds, not "up to" — per Ofcom Voluntary Code of Practice

## Frequently Asked Questions

**Q: Will the engineer need to enter my home?**
A: Yes, a ClearWave engineer will install or activate the FTTP connection at the premises. The appointment window is typically half a day (AM or PM).

**Q: What happens to my speed if everyone is online at once?**
A: As a dedicated FTTP connection, Fibre 100 is not shared with neighbours. Performance during peak times is significantly more consistent than ADSL or cable products.

**Q: Can I keep my existing phone number?**
A: Yes, if the customer is also taking a [PROD-HomePhone](PROD-HomePhone.md) line, their existing number can be ported. Allow up to 5 working days.

**Q: What if my speed is lower than advertised?**
A: If the customer's actual speed is consistently below the minimum guaranteed speed (stated in their contract), they are entitled to a free engineer visit and, if unresolved, to exit the contract without penalty.

## Common Objections

| Objection | Suggested Response |
|-----------|-------------------|
| "It's too expensive" | Fibre 100 is our most affordable full-fibre option. Unlike cheaper copper-based products, the speed is consistent and doesn't degrade with distance from the exchange. Would you like me to check if there's a current promotion? |
| "I don't need that speed" | For a household with even one person streaming in HD and another browsing, 100 Mbps means no buffering or slowdowns. It's the right foundation even for lighter users. |
| "The contract is too long" | We do offer an 18-month term — the monthly cost is slightly higher, but there's less commitment. I can quote both so you can compare. |
| "My current provider is cheaper" | Can I ask what speed you're currently getting? Many customers find they're on a slower copper service paying a similar price without realising full-fibre is available. |

---

## Related

- [PROD-Fibre500Broadband](PROD-Fibre500Broadband.md)
- [PROD-Fibre1000Broadband](PROD-Fibre1000Broadband.md)
- [PROD-HomePhone](PROD-HomePhone.md)
